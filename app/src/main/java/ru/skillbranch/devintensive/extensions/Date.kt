...
fun Date.humanizeDiff(date: Date = Date()): String {
    var diffTime = (this.time - date.time).toInt()
    ...
    if (diffTime < 0) {
        ...
        return when (diffTime) {
            ...
            in 76..2700 -> "${TimeUnits.MINUTE.plural(diffTime / 60)} назад"
            in 2701..4500 -> "час назад"
            in 4501..79200 -> "${TimeUnits.HOUR.plural(diffTime / 3600)} назад"
            in 79201..93600 -> "день назад"
            in 93601..3110400 -> "${TimeUnits.DAY.plural(diffTime / 86400)} назад"
        ...
        return when (diffTime) {
            ...
            in 76..2700 -> "через ${TimeUnits.MINUTE.plural(diffTime / 60)}"
            in 2701..4500 -> "через час"
            in 4501..79200 -> "через ${TimeUnits.HOUR.plural(diffTime / 3600)}"
            in 79201..93600 -> "через день"
            in 93601..3110400 -> "через ${TimeUnits.DAY.plural(diffTime / 86400)}"
